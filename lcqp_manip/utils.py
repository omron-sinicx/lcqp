# this code is copied and modified from pybullet-planning (https://github.com/caelan/pybullet-planning) that is distributed under the MIT license.
...
HALF_CYLINDER_URDF = join_paths(get_parent_dir(__file__), 'half_cylinder_description/half_cylinder.urdf')
...
def create_table(width=0.6, length=1.2, height=0.73, thickness=0.03, radius=0.015,
                 top_color=LIGHT_GREY, leg_color=TAN, cylinder=True, box=True, **kwargs):
    # TODO: table URDF
    ...
    if box:
        ...
        box_thickness = 0.1
        box_height = 0.5
        box_geom = get_box_geometry(width=box_width, length=box_thickness, height=box_height)
        box_pose = Pose(Point(x=0.6, y=y0-box_thickness/2, z=height-thickness/2+box_height/2))
        geoms = geoms + [box_geom]
        poses = poses + [box_pose]
        colors = colors + [top_color]
...
def create_shelf(width=0.3, length=0.6, height=0.3, thickness=0.03,
                 color=DARK_GREY, **kwargs):
    # TODO: table URDF
    surface_top = get_box_geometry(width, width, thickness)
    surface_top_pose = Pose(Point(z=height-thickness/2.))
    surface_right = get_box_geometry(width, thickness, height)
    surface_right_pose = Pose(Point(y=width/2., z=height/2))
    surface_left = get_box_geometry(width, thickness, height)
    surface_left_pose = Pose(Point(y=-width/2., z=height/2))

    geoms = [surface_top, surface_right, surface_left] 
    poses = [surface_top_pose, surface_right_pose, surface_left_pose] 
...
def create_half_cylinder():
    return p.loadURDF(HALF_CYLINDER_URDF, [0, 0, 0], useFixedBase=False)