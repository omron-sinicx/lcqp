...
class Contact:
    """ A set of the complementarity constraints that expresses a contact 
    ...
    def __init__(self, contact_point, contact_surface, contact_name: str, 
        ...
        """ Construct a set of the contact complementarity constraints. 
        ...
        self.f = casadi.vertcat(self.fn, self.ft_plus, self.ft_minus, self.nu) 
        # for animation
        self.f_arrow = None
    ...
    def get_opt_var_bounds(self):
        ...
        c = casadi.vertcat(self.fn, self.ft_plus, self.ft_minus)
        lb = - self.fmax
        ub = self.fmax
        return c, lb, ub
    ...
    def get_contact_complementarity_constraint(self):
        ...
        dx_local = dp_local[0]
        dy_local = dp_local[1]
        cp = casadi.vertcat(y_local+dy_local-self.contact_surface.lb, self.nu, self.nu+dx_local, self.nu-dx_local)
        cf = casadi.vertcat(self.fn, self.mu*self.fn-self.ft_plus-self.ft_minus, self.ft_plus, self.ft_minus)
        return cp, cf
    ...
    def get_contact_dist(self):
        ...
        x_local = p_local[0]
        y_local = p_local[1] + self.contact_point.offset - self.contact_surface.offset
        d = casadi.Function('y_local', [self.q], [y_local])(self.q_val) 
        return d
    ...
    def update_anim(self, f, f_scaling: float=1):
        ...
        if self.contact_point.q is not None:
            ...
            p = self.contact_point.p
        self.f_arrow.set_data((p[0], p[0]+f_scaling*0.1*f_world[0]), (p[1], p[1]+f_scaling*0.1*f_world[1]))